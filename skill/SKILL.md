...
- Single SQLite index at `~/.pawrecall/history.db`, kept fresh by scheduled
  incremental reindexing (or run `python3 ~/pawrecall/pawrecall.py index` manually)
- Contains conversation text only (user + assistant messages), not tool-output logs

## Preferred: MCP tools (pawrecall server)

1. `paw_search(query, source?, limit?)` — full-text search; returns source,
   project path, timestamp, snippet
2. `paw_read(session_id or file_path, limit?)` — read one session back in order

## Fallback: CLI (when MCP tools are unavailable, run via bash)

```bash
python3 ~/pawrecall/pawrecall.py search "keyword" [--source claude|codex|opencode|cursor] [--limit 20]
```

## Workflow

0. **Scope first (data isolation)**: by default pass `project=<current directory name or path substring>` so you only search the current project's history — never scan other projects unless the user explicitly asks for cross-project/global search. Browse what was discussed in a project with `paw_sessions`; replay one exact conversation with `paw_read`
...
3. Use `paw_read` when deeper context is needed
4. Zero results: try synonyms / more specific words; if the index may be stale
   (user just finished a session), reindex first: `python3 ~/pawrecall/pawrecall.py index`
